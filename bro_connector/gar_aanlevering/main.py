...
kvk = {'FR':'01178978', 'GR':'01182023', 'DR':'01179514', 'NH': '34362354', 'FL':'32164140', 'OV':'51048329', 'ZH':'27375169', 'UT':'30277172', 'GE':'51468751', 'ZE':'20168636', 'NB':'17278718', 'LB':'14072118'}
...
def generate_xml_file(csv_name):
    ...
    with tqdm(total=len(unique_gml), desc="Converting Data to Correct Format", initial=0, unit_scale=True, colour='green') as pbar:
        for idx, gml in enumerate(unique_gml):
            if idx <=len(unique_gml):
                ...
                data['delivery_accountable_party'] = sub_list.at[sub_list.index[0],'AccountableParty']
                ...
                data['field_samples']['Paramnummer'] = sub_list_field.loc[:,'Paramnummer'].values.tolist()
                data['field_samples']['Eenheid'] = sub_list_field.loc[:,'Eenheid'].values.tolist()
                data['field_samples']['MeasurementValue'] = sub_list_field.loc[:,'MeasurementValue'].values.tolist()
                data['field_samples']['Qualitycontrolstatus'] = sub_list_field.loc[:,'Qualitycontrolstatus'].values.tolist()

                data['lab_samples'] = {}
                sub_list_lab = sub_list.loc[sub_list['Field2Lab1'] == 1]
                
                data['lab_samples']['Paramnummer'] = sub_list_lab.loc[:,'Paramnummer'].values.tolist()
                data['lab_samples']['Eenheid'] = sub_list_lab.loc[:,'Eenheid'].values.tolist()
                data['lab_samples']['MeasurementValue'] = sub_list_lab.loc[:,'MeasurementValue'].values.tolist()
                data['lab_samples']['Qualitycontrolstatus'] = sub_list_lab.loc[:,'Qualitycontrolstatus'].values.tolist()
                data['lab_samples']['Limietsymbool'] = sub_list_lab.loc[:,'Limietsymbool'].values.tolist()
    ...
    return print("Elapsed time during the whole program in seconds:",t1_stop-t1_start)
