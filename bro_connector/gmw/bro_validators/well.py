...
def validate_well_static(well: GroundwaterMonitoringWellStatic) -> tuple[bool, str]:
    ...
    if well.quality_regime == "IMBRO":
        ...
        # delivery_context
        if well.delivery_context not in [item for subtuple in DELIVERYCONTEXT_IMBRO for item in subtuple]:
            valid = False
            report += f"{well._meta.get_field('delivery_context').verbose_name} moet aangevuld worden om het IMBRO te maken\n"

        # construction_standard
        if well.construction_standard not in [item for subtuple in CONSTRUCTIONSTANDARD_IMBRO for item in subtuple]:
            valid = False
            report += f"{well._meta.get_field('construction_standard').verbose_name} moet aangevuld worden om het IMBRO te maken\n"

        # construction_standard
        if well.initial_function not in [item for subtuple in INITIALFUNCTION_IMBRO for item in subtuple]:
            valid = False
        ...
        if well.horizontal_positioning_method not in [item for subtuple in HORIZONTALPOSITIONINGMETHOD_IMBRO for item in subtuple]:
            valid = False
        ...
        if well.local_vertical_reference_point not in [item for subtuple in LOCALVERTICALREFERENCEPOINT for item in subtuple]:
            valid = False
        ...
        # vertical_datum
        if well.vertical_datum not in [item for subtuple in VERTICALDATUM for item in subtuple]:
            valid = False
        ...
        # delivery_context
        if well.delivery_context not in [item for subtuple in DELIVERYCONTEXT for item in subtuple]:
            valid = False
            report += f"{well._meta.get_field('delivery_context').verbose_name} moet aangevuld worden om het IMBRO/A te maken\n"

        # construction_standard
        if well.construction_standard not in [item for subtuple in CONSTRUCTIONSTANDARD for item in subtuple]:
            valid = False
            report += f"{well._meta.get_field('construction_standard').verbose_name} moet aangevuld worden om het IMBRO/A te maken\n"
        # construction_standard
        if well.initial_function not in [item for subtuple in INITIALFUNCTION for item in subtuple]:
            valid = False
        ...
        # horizontal_positioning_method
        if well.horizontal_positioning_method not in [item for subtuple in HORIZONTALPOSITIONINGMETHOD for item in subtuple]:
            valid = False
...
def validate_well_dynamic(well_state: GroundwaterMonitoringWellDynamic) -> tuple[bool, str]:
    valid = True
    report = ""
    ...
    if well_state.groundwater_monitoring_well_static.quality_regime == "IMBRO":
        ...
        # vertical_datum
        if well_state.ground_level_stable not in [item for subtuple in BOOLEAN_CHOICES_IMBRO for item in subtuple]:
            valid = False
        ...
        # well_head_protector
        if well_state.well_head_protector not in [item for subtuple in WELLHEADPROTECTOR_IMBRO for item in subtuple]:
            valid = False
        ...
        if well_state.ground_level_positioning_method not in [item for subtuple in GROUNDLEVELPOSITIONINGMETHOD_IMBRO for item in subtuple]:
            valid = False
        ...
        # vertical_datum
        if well_state.ground_level_stable not in [item for subtuple in BOOLEAN_CHOICES for item in subtuple]:
            valid = False
        ...
        # well_head_protector
        if well_state.well_head_protector not in [item for subtuple in WELLHEADPROTECTOR for item in subtuple]:
            valid = False
        ...
        # ground_level_positioning_method
        if well_state.ground_level_positioning_method not in [item for subtuple in GROUNDLEVELPOSITIONINGMETHOD for item in subtuple]:
            valid = False
