...
def validate_tube_static(tube: GroundwaterMonitoringTubeStatic) -> tuple[bool, str]:
    ...
    if tube.groundwater_monitoring_well_static.quality_regime == "IMBRO":
        ...
        # tube_type
        if tube.tube_type not in [item for subtuple in TUBETYPE_IMBRO for item in subtuple]:
            valid = False
        ...
        if tube.artesian_well_cap_present not in [item for subtuple in BOOLEAN_CHOICES for item in subtuple]:
            valid = False
        ...
        if tube.sediment_sump_present not in [item for subtuple in BOOLEAN_CHOICES for item in subtuple]:
            valid = False
            report += f"{tube._meta.get_field('sediment_sump_present').verbose_name} moet aangevuld worden om het IMBRO te maken\n"
        
        # tube_material
        if tube.tube_material not in [item for subtuple in TUBEMATERIAL_IMBRO for item in subtuple]:
            valid = False
        ...
        # sock_material
        if tube.sock_material not in [item for subtuple in SOCKMATERIAL_IMBRO for item in subtuple]:
            valid = False
        ...
        # tube_material
        if tube.tube_material not in [item for subtuple in TUBEMATERIAL for item in subtuple]:
            valid = False
        ...
        # sock_material
        if tube.sock_material not in [item for subtuple in SOCKMATERIAL for item in subtuple]:
            valid = False
...
def validate_tube_dynamic(
    ...
    if (
        ...
        # variable_diameter
        if tube_state.variable_diameter not in [item for subtuple in BOOLEAN_CHOICES for item in subtuple]:
            valid = False
            report += f"{tube_state._meta.get_field('variable_diameter').verbose_name} moet aangevuld worden om het IMBRO te maken\n"

        # tube_status
        if tube_state.tube_status not in [item for subtuple in TUBESTATUS_IMBRO for item in subtuple]:
            valid = False
        ...
        if tube_state.tube_top_positioning_method not in [item for subtuple in TUBETOPPOSITIONINGMETHOD_IMBRO for item in subtuple]:
            valid = False
        ...
        if tube_state.tube_packing_material not in [item for subtuple in TUBEPACKINGMATERIAL_IMBRO for item in subtuple]:
            valid = False
            report += f"{tube_state._meta.get_field('tube_packing_material').verbose_name} moet aangevuld worden om het IMBRO te maken\n"

        # glue
        if tube_state.glue not in [item for subtuple in GLUE_IMBRO for item in subtuple]:
            valid = False
        ...
        # tube_status
        if tube_state.tube_status not in [item for subtuple in TUBESTATUS for item in subtuple]:
            valid = False
        ...
        # tube_top_positioning_method
        if tube_state.tube_top_positioning_method not in [item for subtuple in TUBETOPPOSITIONINGMETHOD for item in subtuple]:
            valid = False
        ...
        # tube_packing_material
        if tube_state.tube_packing_material not in [item for subtuple in TUBEPACKINGMATERIAL for item in subtuple]:
            valid = False
