...
def find_well(well_coordinates: Point, nitg: str) -> GroundwaterMonitoringWellStatic:
    if (nitg != None) and (nitg != "[onbekend]"):
...
def create_monitoring_well(df: pl.DataFrame):
    ...
    for row in df.iter_rows(named=True):
        ...
        if row["Opgeruimd"] == "Ja":
            ...
            elif (nitg_code == "[onbekend]") and (well == None):
                unknown += 1
                # logica:
                # maak put aan, gebruik __str__() daarna om de internal_id te updaten als dat kan
                # maak één tube aan als dat nog niet erin zit
            ...
            elif (nitg_code[0] == "P") and (well == None):
                surface += 1
                # logica:
                # maak put aan, gebruik __str__() daarna om de internal_id te updaten als dat kan
            ...
            elif (nitg_code[0] == "B") and (well == None):
                normal += 1
                # logica:
    ...
    som = opgeruimd+dino+dino_e+unknown+unkown_e+surface+surface_e+normal+normal_e
    print("")
