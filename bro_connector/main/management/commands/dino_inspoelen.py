...
from gmn.models import GroundwaterMonitoringNet, GroundwaterMonitoringTubeStatic, MeasuringPoint
import polars as pl
import pandas as pd
import os
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        df_ouput = pd.DataFrame({'put':[], 'peilbuis':[], 'put toegevoegd aan database':[], 'peilbuis toegevoegd aan database':[]})
        ...
        DINO_2 = DINO_1[(DINO_1['X-Coordinaat (RD)'] > BBOX_SETTINGS["xmin"]) &
        ...
        for well_nitg in unique_wells_nitg:
            ...
            if well: # if a well is found
                ...
                for i in range(len(DINO_5)):
                    ...
                    if tube: # tube found
                        output_row = {'put':dino_row['NITG-Nr'], 'peilbuis':dino_row['Buis-Nr'], 'put toegevoegd aan database':'nee', 'peilbuis toegevoegd aan database':'nee'}
