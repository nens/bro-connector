...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "meetnet_naam",
            type=str,
            help="Hoe moet het meetnet heten?")
    ...
    def handle(self, *args, **options):
        ...
        if meetnet_exists:
            print(f'Het meetnet {meetnet_naam} bestaat al. Verwijder dit object of kies een andere naam.')
            return
        else:
        ...
        print(f"{len(gmw_tubes)} filters gevonden in de database. Hiervoor worden Measuring Points aangemaakt, gelinkt aan het '{meetnet_naam}' Meetnet")
