...
class Command(BaseCommand):
    def handle(self, *args, **options):
        for observation in Observation.objects.filter(observation_endtime__isnull=True):
            ...
            first_mtvp = MeasurementTvp.objects.filter(
            ...
            if first_mtvp.measurement_time < observation.observation_starttime:
                ...
                logger.info(f"Old date: {observation.observation_starttime}; New date: {first_mtvp.measurement_time - datetime.timedelta(seconds=1)}.")

                with reversion.create_revision():
                    observation.observation_starttime = first_mtvp.measurement_time - datetime.timedelta(seconds=1)
