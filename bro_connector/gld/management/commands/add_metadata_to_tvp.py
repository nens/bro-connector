...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        measurements_qs = MeasurementTvp.objects.filter(
        ...
        offset = 0
        while True:
            batch = list(measurements_qs[offset : offset + BATCH_SIZE])
            if not batch:
                break
            ...
            with transaction.atomic():
                ...
                metadata_objects = [
                    MeasurementPointMetadata(status_quality_control="nogNietBeoordeeld")
                    for _ in batch
                ]
