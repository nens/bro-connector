...
from gld.models import Observation, Organisation
from main.settings.base import KVK_USER


def get_organisation() -> Organisation:
    return Organisation.objects.get_or_create(company_number=KVK_USER)[0]
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        qs = Observation.objects.annotate(
        ...
        for observation in qs:
            print(
                f"Processing observation {observation} with {observation.measurement.count()} measurements"
            )
            first_measurement = observation.measurement.order_by(
                "measurement_time"
            ).first()
            if (
                first_measurement
                and observation.observation_starttime
