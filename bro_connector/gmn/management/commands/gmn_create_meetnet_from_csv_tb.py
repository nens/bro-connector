...
def update_or_create_meetnet(
    ...
    print("")
    print(
        f"{meetnet_naam} zou {len(df_sbb) + len(df_nm) + len(df_hzl)} peilbuizen moeten bevatten"
    )
    ...
    print(
        f"ssb: {already_in_meetnet_ssb} were already in the meetnet out of {len(df_sbb)}\t {added_sbb} were newly added"
    )
    print(
        f"nm: {already_in_meetnet_nm} were already in the meetnet out of {len(df_nm)}\t {added_nm} were newly added"
    )
    print(
        f"hzl: {already_in_meetnet_hzl} were already in the meetnet out of {len(df_hzl)}\t {added_hzl} were newly added"
    )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        df_hzl = pl.read_csv(
            csv_path + "\Peilbuizen HZL BRO TNO lijst.csv", ignore_errors=True
        )
        ...
        update_or_create_meetnet(
            df_sbb, df_nm, df_hzl, "terreinbeheerders", output_path
        )
