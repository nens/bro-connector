...
def update_or_create_meetnet(
    ...
    print("")
    print(
        f"voor {len(df_ouput)} putten werd een poging gedaan om het toe te voegen aan het meetnet"
    )
    print(
        f"bij {df_ouput.select(pl.sum('in BRO')).item()} is dit ook daadwerkelijk gelukt waarvan {already_in_meetnet} er al in stonden"
    )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        df = pl.read_csv(
            csv_path + "\Export grondwatermeetnet Functies xGMNx.csv",
            ignore_errors=True,
        )
