...
def create_measuring_point(
    gmn: GroundwaterMonitoringNet, tube: GroundwaterMonitoringTubeStatic, subgroup=False
) -> MeasuringPoint:
    if not subgroup:
        measuring_point = MeasuringPoint.objects.update_or_create(
            gmn=gmn, groundwater_monitoring_tube=tube, code=tube.__str__()
        )[0]
    else:
        measuring_point = MeasuringPoint.objects.update_or_create(
            gmn=gmn,
...
def find_measuringpoint(
    gmn: GroundwaterMonitoringNet, tube: GroundwaterMonitoringTubeStatic
) -> MeasuringPoint:
...
def update_or_create_meetnet(
    df: pl.DataFrame, meetnet_naam: str, ouput_path: str
) -> None:
    ...
    df_ouput = pl.DataFrame(
        {"put": [], "peilbuis": [], "in BRO": []},
        schema={"put": pl.String, "peilbuis": pl.String, "in BRO": pl.Int64},
    )
    ...
    for row in df.iter_rows(named=True):
        ...
        if net == 0:
            ...
            print(
                f"{well_code}-{tube_nr}: \tnet:{net}\tP1:{p1}\tP2:{p2}\tP3:{p3}\tP4:{p4}\tP5:{p5},"
            )
    ...
    print("")
    print(
        f"voor {len(df_ouput)} tubes werd een poging gedaan om het toe te voegen aan het meetnet"
    )
    print(
        f"bij {df_ouput.select(pl.sum('in BRO')).item()} is dit ook daadwerkelijk gelukt"
    )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        df_2021 = pl.read_csv(
            csv_path + "\GWMeetnetZld Bemonsteringsronde 2021 tbv json FieldForm.csv",
            ignore_errors=True,
        )
        df_2024 = pl.read_csv(
            csv_path
            + "\Bijlage 1 GWMeetnetZld Bemonsteringsronde 2024 tbv offerte.csv",
            ignore_errors=True,
        )
        ...
        print(
            "A meaning it is already in the gmn (or gmn with subgroup), C that it is created"
        )
        print(
            "0 meaning that the tube was not found, - that the tube was found but nothing had to be done for this subgroup"
        )
