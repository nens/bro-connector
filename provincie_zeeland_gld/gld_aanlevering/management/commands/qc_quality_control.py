...
def jump_check(measurement_value_difference, measurement_time_difference, jump_slope_limit):
...
def QC_check_main(observation, qc_settings):
    ...
    timeseries_set = models.MeasurementTimeSeries.objects.get(observation_id=observation.observation_id)
    measurement_time_series_id = timeseries_set.measurement_time_series_id 
    timeseries_set = models.MeasurementTvp.objects.filter(measurement_time_series_id=measurement_time_series_id)
    field_value_division_dict = {"cm": 100, "mm": 1000}
    ...
    for tvp in timeseries_set:
        ...
        waterstand_waarde_converted = (waterstand_waarde / field_value_division_dict[tvp.field_value_unit]) # cm or mm to m
        measurement_data = {
    ...
    try: 
        gwl_dossier = models.GroundwaterLevelDossier.objects.get(groundwater_level_dossier_id = gwl_dossier_id)
        gw_monitoring_tube_id = gwl_dossier.groundwater_monitoring_tube_id
        # print(gw_monitoring_tube_id)
        gw_monitoring_tube = models.GroundwaterMonitoringTubes.objects.get(groundwater_monitoring_tube_id = gw_monitoring_tube_id)
        ...
        tube_bottom_position = tube_top_position - (screen_length + plain_tube_part_length)
    except: 
        print("Observation id:", observation.observation_id, "is niet gekoppeld aan een bestaand dossier of buis,",
              "controleer of dossier ", gwl_dossier_id, "bestaat en of de bijbehorende buis bestaat.", 
    ...
    for rij in range(len(measurements_df)): # 1 rij = 1 tvp
        ...
        single_measurement_time = datetime.datetime.strptime(single_measurement_time, '%Y-%m-%dT%H:%M:%S+00:00')
        try:
            previous_measurement_value = measurements_df.loc[rij-1,'value']
            previous_measurement_time = measurements_df.loc[rij-1,'time']
            previous_measurement_time = datetime.datetime.strptime(previous_measurement_time, '%Y-%m-%dT%H:%M:%S+00:00')
            value_difference = abs(single_measurement_value-previous_measurement_value)
        ...
        if rij >= 9:
            ...
            for value_counter in range(10):
                ...
                past_ten_values.append(round(measurements_df.loc[rij-value_counter,'value'],3))
        ...
        # Jump check
        jump_return = jump_check(value_difference,time_difference,qc_settings["jump_slope_limit"])
        ...
        tvp_metadata = models.MeasurementPointMetadata.objects.get(measurement_point_metadata_id=measurements_df.loc[rij,'metadata_id'])
...
class Command(BaseCommand):       
    ...
    def handle(self, *args, **options):
        ...
        for observation in observation_set:
            ...
            afgekeurd_counter, goedgekeurd_counter = QC_check_main(observation, qc_settings)
