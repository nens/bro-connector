...
def order_measurements_list(measurement_list):
    ...
    datetime_values = [datetime.datetime.fromisoformat(tvp['time']) for tvp in measurement_list]
    datetime_ordered = sorted(datetime_values)
    indices = [datetime_values.index(datetime_value) for datetime_value in datetime_ordered]
...
def get_observation_procedure_data(observation_process_id):
    ...
    measurement_instrument_type_data = models.TypeMeasurementInstrumentType.objects.get(id=parameter_measurement_instrument_type_id)
    ...
    if air_pressure_compensation_type is None or air_pressure_compensation_type == 'onbekend':
        observation_procedure_data = {'evaluationProcedure': evaluation_procedure,
...
def generate_gld_addition_sourcedoc_data(observation, 
    ...
    try: 
        ...
        filename = 'GLD_Addition_Observation_{}_GLD_{}.xml'.format(observation.observation_id, gld_bro_id)
...
def gld_addition(additions_dir, acces_token_bro_portal):
    ...
    for observation in observation_set:
        ...
        observation_metadata = models.ObservationMetadata.objects.get(observation_metadata_id=observation_metadata_id)
        observation_type = models.TypeObservationType.objects.get(id=observation_metadata.parameter_measurement_serie_type).value

        if observation.status is None and observation_type == 'controlemeting':
            print('Creating controlemeting sourcedocuments for observation ID {}...'.format(observation.observation_id))
            ...
            generate_gld_addition_sourcedoc_data(observation, observation_source_document_data, measurement_time_series_id, additions_dir, addition_type )    
        
        elif observation.status == 'observation_qc_completed' and observation_type == 'reguliereMeting':
...
class Command(BaseCommand):       
    ...
    def handle(self, *args, **options):
        ...
        gld_addition(additions_dir, acces_token_bro_portal)
